/*
    ...
    Print the resulting 4 element vector.
*/
...
int main()
{
    // Multiply a 4x4 array with vector of size 4,
    ...
    sum=0;
    // find sum of dot product of array & vector + sum
    for(int i=0; i<row; i++)
    {
        for(int j=0; j<row; j++)
        {
            sum = (array[i][j] * vector[i]) + sum;
        }
        product[i] = sum;
        sum=0;
    }
    // print sum dotProduct array * vector + sum
    for(int i=0; i<row; i++)
    {
